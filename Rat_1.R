...
for (i in 1:b) {
  ...
  rlars_model <- rlars(x, y) # Fit RLARS
  active <- rlars_model$active # Get active predictors
  significant_predictors <- rep(0, p) # Initialise the selection vector
  significant_predictors[active] <- 1 # Select the active predictors
